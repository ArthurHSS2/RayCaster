  ...
  private:
    ...
    color ray_color(const Ray& r, int depth, const hittable& world, const std::vector<PointLight>& lights, const color& global_ambient) const {
        ...
        if (world.hit(r, interval(0.001, infinity), rec)) {
			...
			//TEXTURA SOLIDA
			//---------------------------------------------------------------------------------------
        	//double val = sin(rec.p.x * 2.0) * cos(rec.p.y * 2.0) * sin(rec.p.z * 2.0);
        	//if (val > 0) rec.kd = color(1.0, 0.2, 0.2); // Vermelho
        	//else         rec.kd = color(0.2, 0.2, 1.0); // Azul

			//Textura procedural
			//double pattern = sin(rec.p.x * 5) * sin(rec.p.y * 5) * sin(rec.p.z * 5);
			  //  if (pattern > 0) rec.kd = color(1,0,0);
			   // else rec.kd = color(0,0,1);
			
			
			//BUMP MAPING
        	double noise = std::sin(rec.p.x * 73.0) * std::cos(rec.p.y * 97.0) * std::sin(rec.p.z * 53.0);
            noise = noise * 0.5 + 0.5; // Mapeia de [-1,1] para [0,1]
                
                // Vetor de perturbação (direções aleatórias mas suaves)
                Vector3d perturbacao(
                    (noise - 0.5) * 0.2,
                    (noise * 0.6 - 0.3) * 0.2,
                    (noise * 0.3 - 0.15) * 0.2
                );
                
                // Aplica e renormaliza
                rec.normal = (rec.normal + perturbacao).normalizacao();
            ...
            double escala_r = std::max(0.0, 1.0 - rec.kr.x - rec.kt.x);
            double escala_g = std::max(0.0, 1.0 - rec.kr.y - rec.kt.y);
            double escala_b = std::max(0.0, 1.0 - rec.kr.z - rec.kt.z);
            color cor_direta = color(cor_final.x * escala_r, cor_final.y * escala_g, cor_final.z * escala_b);
            
            return cor_direta + parte_refletida + parte_refratada;
