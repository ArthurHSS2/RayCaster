...
private:
    ...
    static SceneData makeEmptySceneData() {
        ...
        scene.camera.lookfrom = Point3d(0, 0, 0);
        scene.camera.lookat   = Point3d(0, 0, -1);
        scene.camera.upVector = Vector3d(0, 1, 0);
        scene.camera.image_width  = 0;
        scene.camera.image_height = 0;
        scene.camera.screen_distance = 1.0;

        scene.globalLight.pos = Point3d(0, 0, 0);
    ...
    static Vector3d parseVector(const Json& j, const std::string& what = "vector") {
        auto v = readTriple(j, what);
        return Vector3d(v[0], v[1], v[2]);
    }

    static Point3d parsePoint(const Json& j, const std::string& what = "point") {
        auto v = readTriple(j, what);
    ...
    static ObjectData parseObject(const Json& node, const std::map<std::string, MaterialData>& materials) {
        ...
        for (const auto& [key, val] : node.asObject()) {
            ...
            if (val.isNumber())      obj.numericData[key] = val.asNumber();
                ...
                Vector3d v = parseVector(val, "object." + key);
                obj.vetorPointData[key] = v;

                if(isPositionKey(key)) obj.relativePos = Point3d(v.getX(), v.getY(), v.getZ());
            }
