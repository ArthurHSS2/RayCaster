...
struct TransformData {
    ...
    Vector3d data;    // (dx, dy, dz) | (x, y, z) | (alpha, theta, gama)
};
...
struct ObjectData {
    ...
    Point3d relativePos = Point3d(0, 0, 0);
    MaterialData material;
    
    map<string, double> numericData;
    map<string, Vector3d>  vetorPointData;
    ...
    Vector3d   getVetor   (string key){ return vetorPointData[key]; }
    Point3d getPonto   (string key){ auto p = vetorPointData[key]; return Point3d(p.getX(), p.getY(), p.getZ()); }
...
static void printAsArray(ostream& os, Vector3d v) { os << "[" << v.getX() << ", " << v.getY() << ", " << v.getZ() << "]"; }
