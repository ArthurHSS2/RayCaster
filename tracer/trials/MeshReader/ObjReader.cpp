...
/*
Classe leitora de arquivos .obj. Onde o arquivo contém os vários Point3ds, normais e faces do objeto. No projeto 
trabalhamos com faces triangulares, ou seja, uma face consiste em 3 Point3ds. 
...
Nessa classe podem ser obtidas as seguintes informações (por meio dos Getters):
    - Point3ds
    - Normais
    - Lista de faces com seus respectivos Point3ds
    - Informações de cor, brilho, opacidade, etc.

Obs: -  Para fins de abstração, as normais de cada Point3d são ignoradas e assumimos apenas uma normal para cada face. 
...
private:
    ...
    std::vector<Point3d> vertices;                // Lista de Point3ds
    std::vector<Vector3d> normals;                 // Lista de normais
    std::vector<FaceData> faces;                    // Lista de indices de faces
    std::vector<std::vector<Point3d>> facePoints; // Lista de Point3ds das faces
...
public:
    ...
    // Método para retornar as coordenadas dos Point3ds das faces
    std::vector<std::vector<Point3d>> getFacePoints() {
        return facePoints;
    }

    /*
    Retorna uma lista com um struct faces do objeto. Cada face contém:
        - Índices dos Point3ds
        - Índices das normais
        - Cores (ka, kd, ks, ke)
        - Brilho (ns)
    ...
    */
    ...
    // Método para retornar as coordenadas dos Point3ds
    ...
    // Emite um output no terminal para cada face, com seus respectivos Point3ds (x, y, z)
    void print_faces() {
        int i = 0;
