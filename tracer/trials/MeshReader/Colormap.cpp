...
struct MaterialProperties {
    Vector3d  kd;     // Difuso      (kd)
    Vector3d  ks;     // Especular   (ks)
    Vector3d  ka;     // Ambiente    (ka)
    Vector3d  kr;     // Reflexivo   (kr)  [Emissivo  (ke)] considere ke = kr
    Vector3d  ke;     // Emissivo    (ke)  [Reflexivo (kr)] considere kr = ke  //normalmente o ke é o mais usado
    Vector3d  kt;     // Transmicivo (kt)
    ...
    colormap(string input){
        ...
        while (std::getline(mtlFile, line)) {
            ...
            if (keyword == "newmtl") {
                ...
                iss >> kdR >> kdG >> kdB;
                if (!currentMaterial.empty()) {
                    mp[currentMaterial].kd = Vector3d(kdR, kdG, kdB);
                }
            } else if (keyword == "Ks") {
                double ksR, ksG, ksB;
                iss >> ksR >> ksG >> ksB;
                if (!currentMaterial.empty()) {
                    mp[currentMaterial].ks = Vector3d(ksR, ksG, ksB);
                }
            } else if (keyword == "Ke") {
                double keR, keG, keB;
                iss >> keR >> keG >> keB;
                if (!currentMaterial.empty()) {
                    mp[currentMaterial].ke = Vector3d(keR, keG, keB);
                }
            } else if (keyword == "Kr") {
                double krR, krG, krB;
                iss >> krR >> krG >> krB;
                if (!currentMaterial.empty()) {
                    mp[currentMaterial].kr = Vector3d(krR, krG, krB);
                }
            }
            else if (keyword == "Ka") {
                double kaR, kaG, kaB;
                iss >> kaR >> kaG >> kaB;
                if (!currentMaterial.empty()) {
                    mp[currentMaterial].ka = Vector3d(kaR, kaG, kaB);
                }
            } else if (keyword == "Ns") {
